...
export default function BookingForm() {
  ...
  const validate = () => {
    ...
    if (form.name.trim().length < 2) {
      nextErrors.name = "Укажите имя от 2 символов.";
    }
    if (phoneDigits.length < 10 || phoneDigits.length > 15) {
      nextErrors.phone = "Укажите корректный телефон.";
    }
    if (form.brand.trim().length < 2) {
      nextErrors.brand = "Укажите марку автомобиля.";
    }
    if (form.model.trim().length < 1) {
      nextErrors.model = "Укажите модель автомобиля.";
    }
    if (!Number.isInteger(year) || year < 1980 || year > currentYear) {
      nextErrors.year = `Год должен быть от 1980 до ${currentYear}.`;
    }
    if (!form.service) {
      nextErrors.service = "Выберите услугу.";
    }
    if (form.comment.length > 500) {
      nextErrors.comment = "Комментарий должен быть короче 500 символов.";
    }
  ...
  return (
    ...
    >
      <div className="mb-7">
        <p className="text-sm font-bold uppercase text-signal">Запись на сервис</p>
        <h3 className="mt-2 text-2xl font-black text-white">Получите точную смету</h3>
        <p className="mt-3 text-sm leading-6 text-white/[0.62]">
          Мастер свяжется с вами, уточнит симптомы и предложит удобное окно.
        </p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          <span className="mb-2 block text-sm font-semibold text-white/[0.82]">Имя</span>
          <input
            className={fieldBase}
            value={form.name}
        ...
        <label className="block">
          <span className="mb-2 block text-sm font-semibold text-white/[0.82]">Телефон</span>
          <input
            className={fieldBase}
            value={form.phone}
        ...
        <label className="block">
          <span className="mb-2 block text-sm font-semibold text-white/[0.82]">Модель</span>
          <input
            className={fieldBase}
            value={form.model}
      ...
      <label className="mt-4 block">
        <span className="mb-2 block text-sm font-semibold text-white/[0.82]">Комментарий</span>
        <textarea
          className={cn(fieldBase, "min-h-28 resize-y py-3")}
          value={form.comment}
          onChange={(event) => updateField("comment", event.target.value)}
          placeholder="Опишите проблему, пробег или удобное время для звонка"
      ...
      {success ? (
        <div
          ...
          aria-live="polite"
        >
          Заявка принята. Мы перезвоним в течение 10 минут и подтвердим запись.
        </div>
      ) : null}
